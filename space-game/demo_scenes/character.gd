...
@export var walk_bob_amplitude: float = 0.04
@export var sprint_bob_frequency: float = 14.0
@export var sprint_bob_amplitude: float = 0.06
@export var crouch_bob_frequency: float = 7.0
@export var crouch_bob_amplitude: float = 0.025
...
var _current_bob_offset_y: float = 0.0
var _current_bob_offset_x: float = 0.0
...
func _update_crouch_height(delta: float):
	...
	camera.position.y = target_cam_height + _current_bob_offset_y

func _update_head_bob(delta: float):
	if not bob_enabled:
		_bob_offset_y = 0.0
		_bob_offset_x = 0.0
		_current_bob_offset_y = lerp(_current_bob_offset_y, 0.0, bob_lerp_speed * delta)
		_current_bob_offset_x = lerp(_current_bob_offset_x, 0.0, bob_lerp_speed * delta)
		return

	var should_bob = movement_mode == MovementMode.GROUNDED and is_on_floor()
	var horizontal_speed = Vector3(velocity.x, 0, velocity.z).length()

	if should_bob and horizontal_speed > 0.5 and ground_state != GroundState.SLIDING:
		var freq: float
		var amp: float

		match ground_state:
			GroundState.SPRINTING:
				freq = sprint_bob_frequency
				amp = sprint_bob_amplitude
			GroundState.CROUCHING:
				freq = crouch_bob_frequency
				amp = crouch_bob_amplitude
			_:
				freq = walk_bob_frequency
				amp = walk_bob_amplitude

		var speed_factor = clamp(horizontal_speed / walk_speed, 0.0, 1.5)
		_bob_timer += delta * freq * speed_factor
		_bob_offset_y = sin(_bob_timer) * amp * speed_factor
		_bob_offset_x = cos(_bob_timer * 0.5) * amp * bob_horizontal_ratio * speed_factor
	else:
		_bob_timer = 0.0
		_bob_offset_y = 0.0
		_bob_offset_x = 0.0

	_current_bob_offset_y = lerp(_current_bob_offset_y, _bob_offset_y, bob_lerp_speed * delta)
	_current_bob_offset_x = lerp(_current_bob_offset_x, _bob_offset_x, bob_lerp_speed * delta)

	camera.position.x = _current_bob_offset_x
